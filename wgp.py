...
# Early RTX 5090 memory optimization - must be done immediately after mmgp import
try:
    import torch
    if torch.cuda.is_available():
        ...
        if "RTX 5090" in device_name:
            ...
            if ram_gb >= 90:  # 96GB+ systems
                # Force 70% memory reservation for RTX 5090 systems BEFORE any models load
                offload.perc_reserved_mem_max = 0.70
                print(f"EARLY: RTX 5090 detected - forcing 70% memory reservation ({ram_gb*0.70:.1f}GB of {ram_gb:.0f}GB)")
                
                # GLOBAL SPEED OPTIMIZATIONS for RTX 5090
                torch.backends.cudnn.benchmark = True  # Enable CuDNN auto-tuner
                torch.backends.cuda.matmul.allow_tf32 = True  # Enable TensorFloat-32
                torch.backends.cudnn.allow_tf32 = True  # Enable TF32 for CuDNN
                torch.set_float32_matmul_precision('high')  # Use high precision for speed
                ...
                print("EARLY: RTX 5090 global acceleration enabled - TF32, CuDNN benchmark, aggressive memory")
except:
    pass
...
from wan.utils.utils import extract_audio_tracks, combine_video_with_audio_tracks, cleanup_temp_audio_files, calculate_new_dimensions
...
try:
    if torch.cuda.is_available() and force_profile_no < 0:
        device_name = torch.cuda.get_device_name()
        vram_gb = torch.cuda.get_device_properties(0).total_memory / (1024**3)
        
        # RTX 5090 with 32GB VRAM should use Profile 1 (HighRAM_HighVRAM) for best performance
        if "RTX 5090" in device_name and vram_gb >= 30:
            import psutil
            ram_gb = psutil.virtual_memory().total / (1024**3)
            if ram_gb >= 64:  # If system has 64GB+ RAM
                print(f"RTX 5090 with {ram_gb:.0f}GB RAM detected: Using Profile 1 (HighRAM_HighVRAM) for optimal performance")
                profile = 1
            elif ram_gb >= 32:  # If system has 32GB+ RAM
                print(f"RTX 5090 with {ram_gb:.0f}GB RAM detected: Using Profile 3 (LowRAM_HighVRAM) for good performance")
                profile = 3
except Exception as e:
    print(f"Profile auto-detection failed: {e}")

compile = server_config.get("compile", "")

# RTX 5090 specific: Disable PyTorch compilation due to Triton compatibility issues with RTX 5090 + PyTorch 2.7 nightly
try:
    if torch.cuda.is_available():
        device_name = torch.cuda.get_device_name()
        if "RTX 5090" in device_name and torch.__version__.startswith("2.7"):
            ...
            # Additional RTX 5090 PyTorch optimizations for faster loading
            torch.backends.cuda.matmul.allow_tf32 = True
            torch.backends.cuda.matmul.allow_fp16_reduced_precision_reduction = True
            torch.backends.cudnn.benchmark = True
            torch.backends.cudnn.allow_tf32 = True
            print("RTX 5090 PyTorch optimizations: Enabled TF32, FP16 reduction, CuDNN benchmark for faster loading")
except:
    pass

# RTX 5090 memory optimization: Increase RAM reservation for large model pinning
try:
    if torch.cuda.is_available():
        device_name = torch.cuda.get_device_name() 
        if "RTX 5090" in device_name:
            import psutil
            ram_gb = psutil.virtual_memory().total / (1024**3)
            if ram_gb >= 90:  # 96GB RAM systems
                from mmgp import offload
                # Increase reserved memory to 70% to handle 14B models (need ~48GB, allows ~67GB)
                offload.perc_reserved_mem_max = 0.70
                # Additional RTX 5090 optimizations for faster loading
                offload.max_split_size_mb = 1024  # Larger splits for faster transfer
                if hasattr(offload, 'enable_fast_loading'):
                    offload.enable_fast_loading = True  # Enable fast loading mode if available
                
                # Debug output to verify settings
                print(f"RTX 5090 + {ram_gb:.0f}GB RAM: Memory reservation set to 70% (~{ram_gb*0.70:.1f}GB)")
                print(f"RTX 5090 optimizations applied: mmgp.perc_reserved_mem_max = {offload.perc_reserved_mem_max}")
            else:
                from mmgp import offload
                offload.perc_reserved_mem_max = 0.70
                print(f"RTX 5090 + {ram_gb:.0f}GB RAM: Increased memory reservation to 70% for large model support")
except Exception as e:
    print(f"RTX 5090 memory optimization failed: {e}")
    pass
...
def load_models(model_type):
    ...
    # RTX 5090 AGGRESSIVE optimization: Maximum preload for fastest cocktail model loading
    if preload == 0:  # If no preload specified
        try:
            if torch.cuda.is_available():
                device_name = torch.cuda.get_device_name()
                vram_gb = torch.cuda.get_device_properties(0).total_memory / (1024**3)
                ...
                if "RTX 5090" in device_name and vram_gb >= 30:
                    # AGGRESSIVE: RTX 5090 has 32GB VRAM - maximize preload for cocktail model 3-file loading
                    if "cocktail" in model_type or "14B" in model_type or base_model_type in ["vace_14B", "t2v", "i2v", "multitalk", "fantasy"]:
                        preload = 24576  # 24GB preload for cocktail models (3 files need more memory)
                        print(f"RTX 5090 + COCKTAIL MODEL: Setting preload to {preload}MB (24GB) for MAXIMUM 3-file loading speed")
                    elif "14B" in model_type:
                        preload = 18432  # 18GB preload for regular 14B models
                        print(f"RTX 5090 + 14B model: Setting preload to {preload}MB (18GB) for maximum loading speed")
                    else:
                        preload = 12288  # 12GB preload for smaller models
                        print(f"RTX 5090 detected: Setting preload to {preload}MB (12GB) for faster model loading")
        except:
            pass
    ...
    # RTX 5090 memory override - prioritize over command line args
    ...
    try:
        if torch.cuda.is_available():
            ...
            if "RTX 5090" in device_name:
                import psutil
                ram_gb = psutil.virtual_memory().total / (1024**3)
                if ram_gb >= 90 and perc_reserved_mem_max == 0:  # Only override if not explicitly set
                    perc_reserved_mem_max = 0.70
                    print(f"RTX 5090 OVERRIDE: Using 70% memory reservation instead of default for {ram_gb:.0f}GB system")
    except:
        pass
    if preload == 0:
        preload = server_config.get("preload_in_VRAM", 0)
    ...
    # RTX 5090 COCKTAIL OPTIMIZATION: Parallel loading preparation
    cocktail_mode = "cocktail" in model_type and len(model_file_list) >= 3
    if cocktail_mode:
        print(f"RTX 5090 COCKTAIL MODE: Preparing parallel loading for {len(model_file_list)} files")
        # Pre-allocate CUDA cache and set memory growth
        torch.cuda.empty_cache()
        if hasattr(torch.cuda, 'set_memory_fraction'):
            torch.cuda.set_memory_fraction(0.95)  # Use 95% of VRAM aggressively
    ...
    # RTX 5090 PERFORMANCE REPORT
    load_time = time.time() - start_time
    if cocktail_mode:
        print(f"RTX 5090 COCKTAIL LOADING COMPLETE: {len(model_file_list)} files loaded in {load_time:.1f}s ({load_time/len(model_file_list):.1f}s per file)")
    else:
        print(f"RTX 5090 MODEL LOADING COMPLETE: {load_time:.1f}s")
    
    kwargs = { "extraModelsToQuantize": None }    
    loras_transformer = ["transformer"]
    if profile in (2, 4, 5):
        # RTX 5090 COCKTAIL OPTIMIZATION: Increase budgets for multi-file models
        base_budget = 100 if preload == 0 else preload
        cocktail_multiplier = 1.5 if cocktail_mode else 1.0
        
        budgets = { 
            "transformer" : int(base_budget * cocktail_multiplier), 
            "text_encoder" : int(base_budget * cocktail_multiplier), 
            "*" : max(1000 if profile==5 else 3000, int(preload * cocktail_multiplier)) 
        }
        if "transformer2" in pipe:
            budgets["transformer2"] = int(base_budget * cocktail_multiplier)
        kwargs["budgets"] = budgets
        
        if cocktail_mode:
            print(f"RTX 5090 COCKTAIL: Applied {cocktail_multiplier}x budget multiplier for 3-file model")
    elif profile == 3:
        kwargs["budgets"] = { "*" : "75%" if cocktail_mode else "70%" }  # More aggressive for cocktail
...
def generate_video(
    ...
    if hasattr(wan_model, "get_loras_transformer"):
        ...
        transformer_loras_filenames += extra_loras_transformers
        transformer_loras_multipliers += extra_loras_multipliers
    ...
    while not abort: 
        ...
        while not abort:
            ...
            if hunyuan_custom or hunyuan_avatar or flux:
                ...
                if image_start != None:
                    ...
                    prefix_video = image_start.resize((new_width, new_height), resample=Image.Resampling.LANCZOS) 
                    prefix_video =  torch.from_numpy(np.array(prefix_video).astype(np.float32)).div_(127.5).sub_(1.).movedim(-1, 0).unsqueeze(1)
                    pre_video_guide =  prefix_video
                    image_start = None
                else:
            ...
            if (vace or t2v or ltxv) and video_guide != None :
...
def generate_video_tab(update_form = False, state_dict = None, ui_defaults = None, model_family = None, model_choice = None, header = None, main = None):
    ...
    with gr.Row():
        ...
        if update_form:
            ...
            gr.on( triggers=[output.change, output.select], fn=select_video, inputs=[state, output], outputs=[last_choice, video_info, video_buttons_row, image_buttons_row, video_postprocessing_tab])
